...
mktemp_fpm() { local t; t=$(mktemp -t fpm.XXXX 2>/dev/null) && echo "$t" || mktemp; }
...
get_pub_cache_dir() { [ -d "$HOME/.pub-cache/git" ] && echo "$HOME/.pub-cache/git" || { [ -d "$HOME/AppData/Local/Pub/Cache/git" ] && echo "$HOME/AppData/Local/Pub/Cache/git"; }; }
...
ask_yes_default() { local ans; printf "%s" "${1:-Proceed?} (Y/n): " >/dev/tty; IFS= read -r ans </dev/tty; case "$ans" in [Nn]*) return 1;; *) return 0;; esac; }
# GitHub API cache with 5-minute TTL and rate limiting
github_api_cache() {
  local cache_key="$1" api_url="$2" cache_file="/tmp/fpm_api_${cache_key//\//_}" ttl=300
  [ -f "$cache_file" ] && [ $(($(date +%s) - $(stat -f%m "$cache_file" 2>/dev/null || stat -c%Y "$cache_file" 2>/dev/null || echo 0))) -lt $ttl ] && { cat "$cache_file"; return; }
  sleep 0.1; curl -s "$api_url" | tee "$cache_file" 2>/dev/null || { [ -f "$cache_file" ] && cat "$cache_file"; }
}
# Get short latest commit for repo path + ref (branch/tag); prints "" on failure  
github_latest_short_sha() { github_api_cache "$1_$2" "https://api.github.com/repos/$1/commits/$2" | awk -F'"' '/"sha":/ {print substr($4,1,7); exit}'; }
# Get current commit hash from pubspec.lock for a git dependency
get_lock_commit() {
  local dep_name="$1" pubspec_lock="${2:-pubspec.lock}"
  [ ! -f "$pubspec_lock" ] || [ ! -r "$pubspec_lock" ] || [ ${#dep_name} -gt 100 ] && return 1
  awk -v dep="$dep_name:" 'NR>2000{exit}
    $0 ~ "^  " dep {found=1; next}
    found && /^    revision:/ && length($2)>6 {print substr($2,1,7); exit}
    found && /^  [a-zA-Z]/ {exit}
  ' "$pubspec_lock" 2>/dev/null
}
# Get current commit hash from pub cache for a git dependency
get_cache_commit() {
  local dep_name="$1" cache_dir="$(get_pub_cache_dir)" cached_path cache_lock="/tmp/fpm_cache.lock"
  [ -z "$cache_dir" ] || [ ${#dep_name} -gt 100 ] && return 1
  (flock -s 201; cached_path=$(find "$cache_dir" -name "$dep_name-*" -type d -maxdepth 2 2>/dev/null | head -1)
   [ -n "$cached_path" ] && [ -d "$cached_path/.git" ] && (cd "$cached_path" && timeout 5 git rev-parse --short HEAD 2>/dev/null)
  ) 201>"$cache_lock" 2>/dev/null
}
# Force upgrade git packages with nuclear failsafe
force_upgrade_git_packages() {
  local pubspec_dir="${1:-.}"
  echo "🔄 Forcing Git package updates..."
  (cd "$pubspec_dir" && flutter pub upgrade 2>/dev/null) || true
  flutter pub upgrade --help 2>/dev/null | grep -q '\--force-upgrade' && { echo "   ⚡ Using --force-upgrade flag"; (cd "$pubspec_dir" && flutter pub upgrade --force-upgrade 2>/dev/null) || true; }
}
# Atomic file operations with flock protection
atomic_file_op() {
  local operation="$1" file_path="$2"; shift 2
  local lock_file="$file_path.lock"
  (flock -x 200; case "$operation" in
    backup) cp "$file_path" "$file_path.backup.$(date +%s)" 2>/dev/null;;
    remove) rm -f "$file_path";;
    write) "$@" > "$file_path.tmp" && mv "$file_path.tmp" "$file_path";;
  esac) 200>"$lock_file" 2>/dev/null || true
}
# Nuclear option: Clear locks and cache for stuck packages
nuclear_cache_clear() {
  local pubspec_dir="${1:-.}" dep_names=("${@:2}") lock_path="$pubspec_dir/pubspec.lock" cache_dir="$(get_pub_cache_dir)"
  echo "💥 Nuclear option: Clearing cache and locks"
  [ -f "$lock_path" ] && { atomic_file_op backup "$lock_path"; echo "   📋 Backed up pubspec.lock"; }
  atomic_file_op remove "$lock_path" && echo "   🗑️  Removed pubspec.lock"
  if [ -n "$cache_dir" ]; then
    if [ ${#dep_names[@]} -gt 0 ]; then
      for dep_name in "${dep_names[@]}"; do find "$cache_dir" -name "$dep_name-*" -type d -exec rm -rf {} \; 2>/dev/null && echo "   🗑️  Cleared cache for $dep_name"; done || true
    else
      rm -rf "$cache_dir"/* 2>/dev/null && echo "   🗑️  Cleared entire Git cache" || true
    fi
  fi
  (cd "$pubspec_dir" && flutter pub get 2>/dev/null) || true
}
get_project_dir() { dirname "$SELECTED_PUBSPEC"; }
get_project_name() { basename "$(dirname "$SELECTED_PUBSPEC")"; }
check_pubspec() { local pubspec="${1:-$SELECTED_PUBSPEC}"; [ -f "$pubspec" ] || { echo "❌ pubspec.yaml not found"; return 1; }; }
flutter_in_project() { local project_dir="${1:-$(get_project_dir)}"; shift; (cd "$project_dir" && flutter "$@"); }
# Extract git dependencies from pubspec.yaml (returns temp file path)
extract_git_deps() {
  local pubspec="${1:-$SELECTED_PUBSPEC}"
  [ ! -f "$pubspec" ] || [ ! -r "$pubspec" ] && return 1
  local git_deps=$(mktemp_fpm)
  awk 'NR>1000{exit} /^[[:space:]]*[^#]*:/{dep_name=$1; gsub(/:/, "", dep_name); if(length(dep_name)>100) dep_name=substr(dep_name,1,100)} 
       /^[[:space:]]*git:/{in_git=1; next} 
       in_git && /^[[:space:]]*url:/{url=$2; gsub(/["]/, "", url); if(length(url)>500) url=substr(url,1,500)} 
       in_git && /^[[:space:]]*ref:/{ref=$2; gsub(/["]/, "", ref); if(length(ref)>100) ref=substr(ref,1,100)} 
       in_git && /^[[:space:]]*[^[:space:]]/ && !/url:/ && !/ref:/ && !/path:/{
           if(dep_name && url && length(dep_name)<100 && length(url)<500) {
               print dep_name "|" url "|" (ref && length(ref)<100 ? ref : "main")
               dep_name=""; url=""; ref=""; in_git=0
           }
       }
       /^[^[:space:]]/ && !/dependencies:/ && !/dependency_overrides:/{in_git=0}' "$pubspec" > "$git_deps" 2>/dev/null || return 1
  echo "$git_deps"
}
# Global error handling and cleanup
cleanup_on_exit() { reset_tty; rm -f /tmp/flutter_pm_* 2>/dev/null || true; }
...
init_environment() {
    ...
    local required_files=("$SCRIPT_DIR/../shared/multiselect.sh" "$SCRIPT_DIR/../shared/cross_platform_utils.sh" "$SCRIPT_DIR/../shared/smart_recommendations.sh")
    for file in "${required_files[@]}"; do
        [ ! -f "$file" ] && { echo "❌ Critical file missing: $(basename "$file")"; echo "💡 Please reinstall Flutter Package Manager"; exit 1; }
        source "$file"
    done
    reset_tty
}
...
validate_flutter_environment() {
    ...
    if [ ${#issues[@]} -gt 0 ]; then
        echo -e "\n🔍 **Project Analysis - Issues Detected**\n========================================"
        for i in "${!issues[@]}"; do echo "⚠️  ${issues[$i]}"; done
        echo -e "\n🔧 **Auto-Fix Available**\nI can automatically fix these issues to ensure optimal Flutter development.\n"
        ask_yes_default "Apply automatic fixes?" && apply_auto_fixes "$project_dir" "${fixes[@]}"
    fi
}
...
apply_auto_fixes() {
    ...
    shift
    local fixes=("$@")
    echo "🔧 **Applying Automatic Fixes**"; echo "==============================="
    for fix in "${fixes[@]}"; do
        case "$fix" in
            "create_main_dart")
...
EOF
            ...
            "init_git")
                ...
                cd "$project_dir" && git init >/dev/null 2>&1 && echo "  ✅ Git repository initialized" && cd - >/dev/null
                ;;
            "fix_pubspec")
    ...
    echo ""; echo "✅ **Auto-fixes complete!**"; echo ""
}
...
detect_development_paths() {
    local paths=() common_paths=("$HOME/Development" "$HOME/Projects" "$HOME/dev" "$HOME/workspace" "$HOME/code" ".")
    for path in "${common_paths[@]}"; do [ -d "$path" ] && paths+=("$path"); done
    echo "${paths[@]}"
}
...
get_save_location() {
    echo ""
    echo "📁 Choose save location for GitHub project:"; echo ""
...
get_relative_path() { relpath "$1" "${2:-$(pwd)}"; }
...
update_existing_git_packages_only() {
    echo ""; echo "🔍 Scanning pubspec.yaml for existing Git dependencies..."
    check_pubspec || return 1
    local git_deps; git_deps="$(extract_git_deps)" project_dir; project_dir="$(get_project_dir)"
    if [ ! -s "$git_deps" ]; then echo "ℹ️  No existing Git dependencies found"; rm -f "$git_deps"; return 0; fi
    
    echo "🔍 Checking commit versions..."; local outdated_deps=() all_current=true
    while IFS=\| read -r dep_name git_url git_ref; do
        [ -z "$dep_name" ] || [ -z "$git_url" ] && continue
        local lock_commit="$(get_lock_commit "$dep_name" "$project_dir/pubspec.lock")"
        local cache_commit="$(get_cache_commit "$dep_name")"
        local latest_commit="$(github_latest_short_sha "$(repo_path_from_git_url "$git_url")" "$git_ref")"
        printf "   📦 %-20s %s → %s" "$dep_name:" "${lock_commit:-no-lock}" "${latest_commit:-???}"
        if [ "$lock_commit" != "$latest_commit" ] || [ "$cache_commit" != "$latest_commit" ]; then
            echo " 🔄"; outdated_deps+=("$dep_name|$git_url|$git_ref"); all_current=false
        else echo " ✅"; fi
    done < "$git_deps"
    
    if [ "$all_current" = true ]; then echo ""; echo "✅ All Git dependencies are up to date!"; rm -f "$git_deps"; return 0; fi
    
    echo ""; echo "🔄 Found outdated Git dependencies. Choose update method:"
    echo "1. 📦 Git-only update (flutter pub upgrade) - Default"
    echo "2. 💥 Full nuclear update (clear cache + pubspec.lock)"
    printf "Choice (1-2) [1]: "; read -r choice </dev/tty 2>/dev/null || choice="1"
    
    case "${choice:-1}" in
        2) echo "💥 Nuclear update selected"; nuclear_cache_clear "$project_dir" $(printf "%s\n" "${outdated_deps[@]}" | cut -d'|' -f1);;
        *) echo "📦 Attempting Git-only update..."; force_upgrade_git_packages "$project_dir"
           echo "🔍 Verifying update success..."
           local still_outdated=false
           for dep_info in "${outdated_deps[@]}"; do
               IFS=\| read -r dep_name git_url git_ref <<< "$dep_info"
               local new_lock="$(get_lock_commit "$dep_name" "$project_dir/pubspec.lock")"
               local latest="$(github_latest_short_sha "$(repo_path_from_git_url "$git_url")" "$git_ref")"
               [ "$new_lock" != "$latest" ] && still_outdated=true && break
           done
           if [ "$still_outdated" = true ]; then
               echo "⚠️  Git-only update failed, auto-switching to nuclear option..."
               nuclear_cache_clear "$project_dir" $(printf "%s\n" "${outdated_deps[@]}" | cut -d'|' -f1)
           else echo "✅ Git-only update successful!"; fi;;
    esac
    
    echo "🔍 Final verification..."; local final_mismatches=0
    for dep_info in "${outdated_deps[@]}"; do
        IFS=\| read -r dep_name git_url git_ref <<< "$dep_info"
        local final_lock="$(get_lock_commit "$dep_name" "$project_dir/pubspec.lock")"
        local latest="$(github_latest_short_sha "$(repo_path_from_git_url "$git_url")" "$git_ref")"
        if [ "$final_lock" != "$latest" ]; then
            echo "   ⚠️  $dep_name: still $final_lock (expected $latest)"
            ((final_mismatches++))
        fi
    done
    
    if [ $final_mismatches -gt 0 ]; then
        echo ""; echo "🔧 Last line of defense: updating cached repositories directly..."
        local cache_dir; cache_dir="$(get_pub_cache_dir)"
        if [ -n "$cache_dir" ]; then
            for dep_info in "${outdated_deps[@]}"; do
                IFS=\| read -r dep_name _ _ <<< "$dep_info"
                local cached_path=$(find "$cache_dir" -name "$dep_name-*" -type d -maxdepth 2 2>/dev/null | head -1)
                [ -n "$cached_path" ] && [ -d "$cached_path/.git" ] && {
                    echo "   🔄 Updating cache for $dep_name..."
                    (cd "$cached_path" && timeout 10 git remote update 2>/dev/null) || true
                }
            done
            echo "🔄 Re-running flutter pub get..."; flutter_in_project pub get
        fi
        echo "⚠️  Some packages may still have commit version mismatches."
    else echo "✅ All Git dependencies successfully updated!"
    fi
...
auto_resolve_conflicts() { echo "🔧 Auto-resolving conflicts using dependency_overrides"; }
show_resolution_strategy() { echo "📋 Adding dependency overrides to resolve conflicts"; }
show_detailed_conflict_analysis() { echo "🔍 Conflict analysis: $(echo "$1" | grep -c "depends on") dependency conflicts detected"; }
...
select_packages_to_refresh() {
    ...
    for stale_info in "${stale_deps[@]}"; do
        ...
        if [ "$cached_commit" = "NEW" ]; then
            echo "   • $dep_name (NEW → $latest_commit)"
        else
            echo "   • $dep_name ($cached_commit → $latest_commit)"
        fi
    done
    echo ""
    read -p "🔄 Update all packages? (y/N): " update_choice </dev/tty
...
show_detailed_cache_info() { echo "📋 ${#@} stale Git dependencies found in cache"; }
analyze_package_exports() { echo "🔍 Package analysis: $(basename "$1")"; }
discover_monorepo_structure() { echo "🏗️ Monorepo detected: $(find "$1" -name "pubspec.yaml" | wc -l | tr -d " ") packages found"; }
...
authenticate_github() {
    echo "🔐 Setting up GitHub authentication..."; echo ""
    # Open browser first using cascade fallback
    echo "🌐 Opening GitHub authentication in your browser..."
    if open "https://github.com/login/device" &>/dev/null || (command -v xdg-open &>/dev/null && xdg-open "https://github.com/login/device" &>/dev/null) || open_browser "https://github.com/login/device"; then
        echo "✅ Browser opened successfully"
    else
        echo "❌ Could not open browser automatically."; echo "📝 Please visit: https://github.com/login/device"
    fi
    echo ""; echo "🔑 Starting authentication process..."
    ...
    while kill -0 $GH_PID 2>/dev/null; do
        if [ -f "$TEMP_OUTPUT" ]; then
            ...
            if [ -n "$AUTH_CODE" ] && [ ! -f "/tmp/code_copied_$AUTH_CODE" ]; then
                echo ""; echo "📋 🎯 Authentication Code Found: $AUTH_CODE"; echo ""
    ...
    if [ $GH_EXIT_CODE -eq 0 ] && gh auth status &>/dev/null; then
        echo ""; echo "✅ GitHub authentication successful!"
        return 0
    else
        echo ""; echo "❌ Authentication failed or was cancelled."
        echo ""; echo "🔧 You can try again by running: gh auth login --web"; echo ""
        return 1
    fi
}
...
if ! gh auth status &>/dev/null; then
    ...
    echo ""
    if ! ask_yes_default "🚀 Would you like to authenticate now?"; then
