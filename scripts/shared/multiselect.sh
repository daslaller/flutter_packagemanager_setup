...
# Usage: singleselect "prompt" array_name selected_index_var_name
singleselect() {
    local prompt="$1"
    ...
    draw_menu() {
        ...
        echo "$prompt"
        echo ""
        echo "Use ↑/↓ or j/k to navigate, ENTER to select, q to quit"
        echo ""
        ...
        for ((i=0; i<${#options_ref[@]}; i++)); do
            ...
            if [[ $i -eq $cursor ]]; then
                prefix="► "
                echo -e "\033[7m$prefix${options_ref[i]}\033[0m"
            else
                echo "$prefix${options_ref[i]}"
            fi
        done
